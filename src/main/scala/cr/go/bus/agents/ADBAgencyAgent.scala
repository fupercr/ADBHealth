...
/**
  * An agent for a governmental agency.
  */
class ADBAgencyAgent extends ADBAbstractAgent with ADBForwardRouter with ADBBackwardsRouter
  ...
  /**
    ...
    */
  override val rest = {
    respondWithSingletonHeader(RawHeader("Agency agent", "host: %s, port: %s".format(agentHost, agentPort)))
