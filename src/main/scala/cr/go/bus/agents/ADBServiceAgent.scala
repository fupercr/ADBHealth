...
/**
  * An agent that models a particular service. The philosophy of this framework is
  * one service-one agent type
  */
class ADBServiceAgent extends ADBAbstractAgent with ADBForwardRouter with ADBBackwardsRouter
  ...
  override val rest: Directive0 = {
    respondWithSingletonHeader(RawHeader("Service agent", "host: %s, port: %s".format(agentHost, agentPort)))
